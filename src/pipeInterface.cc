...
bool
PipeInterface::start(const uint16_t port,
...
bool
PipeInterface::send(std::unique_ptr<Packet> packet)
{
  assert(downStream);
...
bool
PipeInterface::fromDownstream(std::unique_ptr<Packet> packet)
{
  assert(upStream);
...
void
PipeInterface::updateStat(PipeInterface::StatName stat, uint64_t value)
{
  if (upStream) {
...
void
PipeInterface::ack(ShortName name)
{
  if (upStream) {
    upStream->ack(name);
  }
...
void
PipeInterface::updateRTT(uint16_t minRtMs, uint16_t bigRtMs)
{
  if (downStream) {
...
void
PipeInterface::updateMTU(uint16_t mtu, uint32_t pps)
{
  if (downStream) {
...
void
PipeInterface::updateBitrateUp(uint64_t minBps,
                               uint64_t startBps,
                               uint64_t maxBps)
{
  if (downStream) {
